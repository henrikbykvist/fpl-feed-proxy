...
async function fetchWithTimeout(url, ms = 3500) {
  ...
  try {
    const r = await fetch(url, { signal: ctrl.signal, headers: { 'User-Agent': 'fpl-feed-proxy/1.0' }, cache: 'no-store' });
...
function pickElementFields(el) {
  return {
    id: el.id, web_name: el.web_name, first_name: el.first_name, second_name: el.second_name,
    team: el.team, element_type: el.element_type, now_cost: el.now_cost,
    selected_by_percent: el.selected_by_percent, minutes: el.minutes, form: el.form,
    points_per_game: el.points_per_game, ict_index: el.ict_index, status: el.status,
    chance_of_playing_next_round: el.chance_of_playing_next_round
  };
}
const pickTeamFields = t => ({ id: t.id, name: t.name, short_name: t.short_name, strength: t.strength });
const pickFixtureFields = fx => ({
  id: fx.id, event: fx.event, kickoff_time: fx.kickoff_time,
...
export default async function handler(req, res) {
  try {
    ...
    const fixturesLimit = Math.max(0, Math.min(parseInt(req.query.fixtures ?? '20', 10), 200));
    const topN = Math.max(0, Math.min(parseInt(req.query.topn ?? '300', 10), 600));
